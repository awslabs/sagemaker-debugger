...
def create_tornasole_hook(output_dir, module=None, hook_type='saveall',
    ...
    if hook_type == 'saveall':
        ...
        get_collection('l_mod').add_module_tensors(module, inputs=True,
                                                   outputs=True)
...
def main():
    ...
    parser.add_argument('--tornasole-frequency', type=int, default=10,
    ...
    parser.add_argument('--hook-type', type=str,
                        choices=['saveall', 'module-input-output',
                                 'weights-bias-gradients'], default='saveall')
    ...
    save_steps = [(i + 1) * args.tornasole_frequency for i in
                  range(args.steps // args.tornasole_frequency)]
    model = Net().to(device)
    hook = create_tornasole_hook(args.tornasole_path, model, hook_type,
