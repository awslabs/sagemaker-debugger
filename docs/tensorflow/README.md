...
```
aws s3 sync s3://tornasole-external-preview-use1/sdk/ts-binaries/tornasole_tensorflow/py3/latest/ tornasole_tensorflow/
...
```
hook = smd.SessionHook(out_dir = 's3://bucket/tornasole_outputs/trial',
...
python examples/tensorflow/scripts/simple.py --lr 100 \
    --scale 100000000000 --save_frequency 9 --steps 100 \
    --smdebug_path ~/ts_outputs/not_good
```

You can monitor the exploding tensors by doing the following
```
python -m smdebug.rules.rule_invoker \
...
python examples/tensorflow/scripts/simple.py --lr 100 \
    --scale 100000000000 --save_frequency 9 --steps 100 \
    --smdebug_path s3://my-ts-test/ts_outputs/not_good
```
...
python examples/tensorflow/scripts/train_imagenet_resnet_hvd.py --clear_log \
    --enable_smdebug --save_weights --save_gradients \
    --step_interval 10 --constant_initializer 0.01 \
...
python examples/tensorflow/scripts/train_imagenet_resnet_hvd.py --clear_log \
    --enable_smdebug --save_weights --save_gradients \
    --step_interval 10 --constant_initializer 0.01 \
    --smdebug_path s3://my-ts-test/ts_outputs/vanishing
```
...
```
import smdebug.tensorflow as smd
smd.SessionHook(out_dir='s3://tornasole-testing/trial_job_dir',
...
```
import smdebug.tensorflow as smd
smd.SessionHook(out_dir='/home/ubuntu/tornasole-testing/trial_job_dir',
                 include_regex=['loss*'])
```
...
```
from smdebug.tensorflow import SessionHook, get_collection, modes, SaveConfig
