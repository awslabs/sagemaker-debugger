...
```hook.set_mode(smd.modes.EVAL)``` or
...
| ```coll.add_block_tensors(block, inputs=False, outputs=False)```  | Takes an instance Gluon block, inputs and outputs flags. Users can use this Collection to log input/output tensors for a specific block. By default this method logs the outputs of the passed block  |
