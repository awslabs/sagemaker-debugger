...
| `save_tensor()`| `tensor_name (str)`, `tensor_value (np.ndarray)`, `collections_to_write (str or list[str])` | Manually save metrics tensors. The `record_tensor_value()` API is deprecated in favor or `save_tensor()`.|
