...
run_for_framework() {
	...
	# integration_testing_rules is flaky in xgb 1.3
        python -m pytest ${code_coverage_smdebug:+--cov=./ --cov-append}  --durations=50 --html=$REPORT_DIR/report_$1.html -v -s --self-contained-html --ignore=tests/core/test_paths.py --ignore=tests/core/test_index_utils.py --ignore=tests/core/test_collections.py --ignore=tests/tensorflow2/test_keras_mirrored.py tests/$1
      else
