...
if [ "$run_pytest_pytorch" = "enable" ] ; then
  python -m pytest ${code_coverage_smdebug:+--cov=./ --cov-append}  --durations=50 --html=$REPORT_DIR/report_$1.html -v -s --self-contained-html tests/core/test_timeline_writer.py::test_utc_timestamp
