...
def exhaustive_check(out_dir, use_tf_keras):
    ...
    assert len(tr.tensornames(collection=CollectionKeys.GRADIENTS)) == 4
    ...
    assert len(tr.tensornames(collection=CollectionKeys.WEIGHTS)) == 2
    assert len(tr.tensornames(collection=CollectionKeys.BIASES)) == 2
    ...
    assert len(tr.tensornames(collection=CollectionKeys.METRICS)) == 3

    if use_tf_keras:
        assert len(tr.tensornames(collection=CollectionKeys.OPTIMIZER_VARIABLES)) == 5
        opt_var_name = tr.tensornames(collection=CollectionKeys.OPTIMIZER_VARIABLES)[0]
