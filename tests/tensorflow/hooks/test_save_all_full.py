...
def test_save_all_full(out_dir, hook=None):
    ...
    assert len(tr.tensornames()) > 50
    print(tr.tensornames(collection="weights"))
    assert len(tr.tensornames(collection="weights")) == 1
    assert len(tr.tensornames(collection="gradients")) == 1
    assert len(tr.tensornames(collection="losses")) == 1
