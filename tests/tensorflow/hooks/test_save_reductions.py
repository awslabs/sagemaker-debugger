...
def helper_save_reductions(trial_dir, hook):
    ...
    num_tensors_collection = (
        len(coll["weights"].tensor_names)
...
def test_save_reductions():
    ...
    rdnc = ReductionConfig(
        reductions=["min", "max", "mean", "prod", "std", "sum", "variance"],
...
def test_save_reductions_json():
    ...
    ] = "tests/tensorflow/hooks/test_json_configs/test_save_reductions.json"
