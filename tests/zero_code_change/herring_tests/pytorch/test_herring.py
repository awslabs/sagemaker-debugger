...
# Third Party
import pytest
from tests.zero_code_change.herring_tests.constants import (
    HERRING_PYTORCH_TEST_MNIST_ARGS,
    HERRING_PYTORCH_TEST_MNIST_SCRIPT,
)
from tests.zero_code_change.herring_tests.utils import launch_herring_job
from tests.zero_code_change.utils import build_json
from torch.cuda import device_count

# First Party
from smdebug.profiler.profiler_config_parser import ProfilerConfigParser
from smdebug.profiler.profiler_constants import DEFAULT_PREFIX, HERRINGTIMELINE_SUFFIX
from smdebug.profiler.tf_profiler_parser import HerringProfilerEvents
...
def mode_allworkers(out_dir, mode):
    ...
    mode_args = list(HERRING_PYTORCH_TEST_MNIST_ARGS)
    launch_herring_job(
        script_file_path=HERRING_PYTORCH_TEST_MNIST_SCRIPT,
...
@pytest.mark.skip(
    reason="Requires Herring docker image which is private as of now. It would be available in general DLC sometime in mid of November 2020"
)
...
def test_mode_workers_dynamic_hvd_profiler(
    ...
    """
        This test is meant to verify dynamically turning ON/OFF Herring profiler with PyTorch.
        :param mode: gpu
    ...
    herring_profiler_config += _convert_key_and_value("StartStep", 2)
    herring_profiler_config += _convert_key_and_value("NumSteps", 1)
    herring_profiler_config += "}"
    ...
    files = []
    for path in Path(out_dir + "/" + DEFAULT_PREFIX).rglob(f"*{HERRINGTIMELINE_SUFFIX}"):
        files.append(path)

    assert len(files) == 8

    trace_file = str(files[0])
    t_events = HerringProfilerEvents()
