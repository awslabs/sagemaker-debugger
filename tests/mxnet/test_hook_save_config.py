...
def test_save_config(hook=None):
  if hook is None:
    ...
    hook = t_hook(out_dir=out_dir, save_config=save_config, include_collections=["ReluActivation", 'weights', 'biases','gradients', 'default'])
...
def test_save_config_hookjson_config():
  ...
  os.environ[TORNASOLE_CONFIG_FILE_PATH_ENV_STR] = 'tests/mxnet/test_json_configs/test_save_config_hookjson_config.json'
