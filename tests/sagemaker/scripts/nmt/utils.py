...
def download(download_path):
    path_to_zip = tf.keras.utils.get_file(
        "spa-eng.zip", origin=_URL, cache_subdir=download_path, extract=True
    )
...
def create_word_pairs(path, num_examples):
    ...
    word_pairs = [
        [preprocess_sentence(w) for w in l.split("\t")]  # pylint: disable=g-complex-comprehension
...
def create_dataset(path_to_file, num_examples, buffer_size, batch_size):
  ...
  """
    ...
    # Create a tf.data dataset
    train_dataset = tf.data.Dataset.from_tensor_slices((inp_train, target_train)).shuffle(
        buffer_size
    )
