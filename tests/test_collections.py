...
def test_export_load():
  ...
  c1 = Collection('default', include=['conv2d'], reduction_config=ReductionConfig())
...
def test_manager_export_load():
  ...
  cm.get('default').include_name('loss')
  cm.add(Collection('trial1'))
  cm.get('trial1').exclude_name('losses')
  cm.add('trial2')
  cm.get('trial2').include_name('total_loss')
  cm.export('cm.ts')
...
def test_manager():
  ...
  assert 'loss' in cm.get('default').include
  assert 'losses' in cm.get('trial1').exclude
  assert 'total_loss' in cm.collections['trial2'].include