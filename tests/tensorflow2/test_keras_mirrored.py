    ...
    if include_workers == "all":
        ...
        if eager:
            assert len(tr.tensor_names()) == (
                6 + 1 + 2 + 5 + 2 + 6 + 1 if is_tf_2_2() else 6 + 1 + 3 + 5
            )
            # 6 weights, 1 loss, 3 metrics, 5 optimizer variables for Tf 2.1, 1 scalar
            # 6 weights, 1 loss, 2 metrics, 5 optimizer variables, 2 model outputs, 6 gradients, 1 scalar for Tf 2.2
        else:
...
def test_save_all(out_dir, tf_eager_mode, workers):
    ...
    print(tr.tensor_names())
    if tf_eager_mode:
        assert len(tr.tensor_names()) == (6 + 2 + 1 + 5 + 1 + 1 + 2 + 8 + 8 if is_tf_2_2() else 6 + 3 + 1 + 5 + 1 + 1 + 2 + 8)
        # weights, metrics, losses, optimizer variables, scalar, model input, model output, layers, gradients
    else:
        assert (
            len(tr.tensor_names())
            == 6
