...
def test_local_event_mode():
    ...
    trial_obj = LocalTrial(
        name=trial_name, dirname=os.path.join(path, trial_name), index_mode=False
    )
...
def test_s3_event_mode():
    ...
    trial_obj = S3Trial(
        name=trial_name, bucket_name=BUCKET, prefix_name=prefix_name, index_mode=False
    )
    assert bool(trial_obj.index_tensors_dict) is False
