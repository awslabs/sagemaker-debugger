...
class TestRules:
    def __init__(self, framework, path_to_config=None, env_dict={},
                 test_case_list=[], test_case_regex=None, out_dir='./'):
        """
        ...
        self.current_commit_path = os.path.join(os.path.dirname(self.out_dir), os.path.basename(self.out_dir))
    ...
    def delete_s3_trials(self):
        ...
        list_req = [ListRequest(Bucket=INTEGRATION_TEST_S3_BUCKET,
                                Prefix=self.s3_trial_prefix)]
    ...
    def _is_test_allowed(self, job):
        ...
        if self.test_case_regex is not None:
            if re.match(self.test_case_regex, job[TEST_NAME_INDEX]):
                ...
                matches_regex = False
            logger.info("Test regex specified, matches_regex is {} testname:{}".format(matches_regex, name))
                        ...
                        for mode_2 in self.local_and_s3:
                            if self.local_and_s3[mode_2]:
                                if mode_2 == 'local_mode':
                                    trial_dir = '{}/trial_{}_{}_{}'\
                                        .format(self.local_trial_dir,
                                                job[TEST_NAME_INDEX].replace('/', '_'),
                                                mode_1, time_stamp)
                                else:
                                    trial_dir = 's3://{}/{}/trial_{}_{}_{}'\
                                        .format(INTEGRATION_TEST_S3_BUCKET,
    ...
    def move_log(self):
        shutil.move(LOG_PATH, os.path.join(self.logs_dir,
                                           'integration_tests_' + self.framework + '.log'))
        ...
        while True:
            ...
            for process in process_list:
                if process not in ended_processes and not process.is_alive():
                    ...
                    logger.info('Process {} ended with exit code {}'
                                     .format(process.name, process.exitcode))
                    process.join()
            sleep(2)

        if exit_code > 0:
            msg = "exit code of pytest run non zero. Please check logs in s3://{}{}" \
                .format(INTEGRATION_TEST_S3_BUCKET, self.current_commit_path)
            assert False, msg
