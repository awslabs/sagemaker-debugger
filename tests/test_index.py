...
def test_index():
    ...
    writer.close()
    import os
    eventfile = [filename for filename in os.listdir('.') if filename.startswith("events.out.tfevents.")]
    indexfile = [filename for filename in os.listdir('.') if filename.startswith("index")]
