...
from smdebug.rules.generic import *
...
def get_rule_arguments(args_dict, classobj):
    ...
    for i in range(2, start):
        ...
        if arg_name not in args_dict:
            raise ValueError(
                "Required argument {} for {} is missing".format(arg_name, classobj.__name__)
            )
        if arg_name == "other_trials":
            other_trials = create_trials(
                args_dict["other_trials"],
                range_steps=(args_dict["start_step"], args_dict["end_step"]),
            )
