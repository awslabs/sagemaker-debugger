...
class TraceEventParser:
    ...
    def get_events_at_timestamp(self, timestamp_from_utc, unit=TimeUnits.MICROSECONDS):
        if not isinstance(unit, TimeUnits):
            return None
        if unit == TimeUnits.NANOSECONDS:
            timestamp_in_nanoseconds = timestamp_from_utc
        else:
            timestamp_in_nanoseconds = convert_utc_timestamp_to_nanoseconds(
                timestamp_from_utc, unit
            )
        result_events = list()
