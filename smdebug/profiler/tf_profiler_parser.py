...
# First Party
from smdebug.profiler.trace_event_file_parser import TIMESCALE_MULTIPLIER, ProcessInfo, TraceEvent


class SMTFProfilerEvents(TraceEvent):
    def __init__(self, trace_file):
        self._trace_json_file = trace_file
        # SMTFProfile events are in micro seconds, we multiply by 1000 to convert to ns
        self._timescale_multiplier_for_ns = TIMESCALE_MULTIPLIER["us"]
        super().__init__()
...
class TFProfilerEvents(TraceEvent):
    def __init__(self, trace_file):
        self._trace_json_file = trace_file
        # TFPrfoiler events in are in ns
