...
from .tensor import TornasoleTFTensor, TensorType
...
class Collection(BaseCollection):
    ...
    def _store_ts_tensor(self, ts_tensor):
        # todo: what if tesnor already existed
        if ts_tensor:
    ...
    def add_tensor(self, arg, name=None):
        ...
        if isinstance(arg, tf.Operation):
            for t in arg.outputs:
                self._store_ts_tensor(TornasoleTFTensor.from_tensor(t))
        elif isinstance(arg, tf.Variable):
            self._store_ts_tensor(TornasoleTFTensor.from_variable(arg, name))
        elif isinstance(arg, tf.Tensor):
            self._store_ts_tensor(TornasoleTFTensor.from_tensor(arg, name))
        elif isinstance(arg, values.MirroredVariable):
            for value in arg._values:
                self._store_ts_tensor(TornasoleTFTensor.from_variable(value))
        else:
            logger.error(
    ...
    def add_reduction_tensor(self, tensor, original_tensor, tornasole_name=None):
        ts_tensor = TornasoleTFTensor.create_reduction(tensor, original_tensor, tornasole_name)
        if ts_tensor:
            self._tensors[ts_tensor.name_in_graph] = ts_tensor
