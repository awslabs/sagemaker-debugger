...
from .collection import get_collection_manager, TornasoleTFTensor, TensorType
...
class TornasoleHook(tf.train.SessionRunHook, BaseHook):
    ...
    def _add_reduction(self, tensor, reduction_name, collection, abs=False):
        ...
        red_tensor = get_tensorflow_reduction(reduction_name, tensor, abs=abs)
    ...
    def _merge_ts_tensor_objects_across_collections(self, tensor, colls_with_tensor):
        if colls_with_tensor:
            ...
            for coll in colls_with_tensor:
                if first_coll is None:
                    first_coll = coll
                    ts_tensor = first_coll.get_tensor(tensor.name)
                else:
    ...
    def _check_and_add_tensor(self, tensor):
        ...
        for coll in self._get_all_collections_to_save():
            ...
            if coll.has_tensor(tensor.name):
                ...
                coll_with_tensor.add(coll)

        self._merge_ts_tensor_objects_across_collections(tensor, coll_with_tensor)
    ...
    def _get_ts_tensor(self, tf_tensor_name, save_collections=None):
        ...
        if save_collections:
            return first(iter(save_collections)).get_tensor(tf_tensor_name)
        else:
            raise RuntimeError(
